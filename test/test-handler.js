...
describe("Test campaigns", function () {
  beforeEach(async function () {
    ...
    CREATE_CLOSE_HANDLER = await state.CREATE_CLOSE_HANDLER();
    MINTER_NONCE_HANDLER = await state.MINTER_NONCE_HANDLER();
    BURNER_HANDLER = await state.BURNER_HANDLER();
    await state.connect(owner).grantRole(CREATE_CLOSE_HANDLER, handler.address);
    await state.connect(owner).grantRole(MINTER_NONCE_HANDLER, handler.address);
    await state.connect(owner).grantRole(BURNER_HANDLER, handler.address);
...
describe("Test staking", function () {
  ...
  it("should set stakingInfo", async function () {
    ...
    await expect(handler.connect(admin).registerStakeInfo(firstCampaignId, start, stop, release)).to.be.revertedWith(
      "Can not register stake after minting shares"
    );

    await state.connect(owner).grantRole(BURNER_HANDLER, owner.address);
    await state.connect(owner).burnToken(firstCampaignId, 1000, owner.address);

    await expect(
      handler.connect(admin).registerStakeInfo(firstCampaignId + 1, start, stop, release)
    ).to.be.revertedWith("Invalid campaign id");

    await ethers.provider.send("evm_setNextBlockTimestamp", [now]);
    await expect(
      handler.connect(admin).registerStakeInfo(firstCampaignId, now - 100, stop, release)
    ).to.be.revertedWith("Invalid timestamp order");
    await expect(handler.connect(admin).registerStakeInfo(firstCampaignId, stop, start, release)).to.be.revertedWith(
      "Invalid timestamp order"
    );
    await expect(handler.connect(admin).registerStakeInfo(firstCampaignId, start, release, stop)).to.be.revertedWith(
      "Invalid timestamp order"
    );
    ...
    await expect(
      handler.connect(admin).registerStakeInfo(firstCampaignId, start, stop, release + 2000)
    ).to.be.revertedWith("Can not change info after staking has started");
  });

  it("should add stake", async function () {
    now = Date.now();
    await expect(handler.connect(backer).stake(firstCampaignId, 100)).to.be.revertedWith(
  ...
  it("should release stake", async function () {
    ...
    await handler.connect(backer).stake(firstCampaignId, 100);
    await expect(handler.connect(backer).releaseStake(firstCampaignId)).to.be.revertedWith(
      "Can not release stake before release date"
    );
    ...
    await expect(handler.connect(backer).releaseStake(firstCampaignId)).to.be.revertedWith("Nothing staked");
  });
  ...
  it("should prevent minting shares before stake end", async function () {
    ...
    await expect(handler.connect(admin).mintCampaignShares(firstCampaignId, 1000, backer.address)).to.be.revertedWith(
      "Staking phase is not over"
    );
